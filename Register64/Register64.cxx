...
public:
	...
	void Execute(itk::Object * object, const itk::EventObject & event) {
		...
		if (registration->GetCurrentLevel() == 0) {
			...
			optimizer->SetMinimumStepLength(optimizer->GetMinimumStepLength() * pow(10, registration->GetNumberOfLevels()));
		} else {
			optimizer->SetMaximumStepLength(optimizer->GetMinimumStepLength() * 2.0);			
			optimizer->SetMinimumStepLength(optimizer->GetMinimumStepLength() * 0.01);			
		}
...
// READ FILES END
	...
	/*string transform_filename(argv[4]);
	unsigned int pos = transform_filename.find_first_of(".");
	transform_filename = transform_filename.substr(0, pos) + ".tfm";*/
...
void RegisterImages_MIAffine(ImageType::Pointer & fixed_image, ImageType::Pointer & moving_image, TransformTypeAffine::Pointer & out_transform) {
	...
	metric->SetNumberOfHistogramBins(1000);
	const unsigned int numberOfPixels = fixed_image->GetBufferedRegion().GetNumberOfPixels();
	const unsigned int numberOfSamples = static_cast<unsigned int>(numberOfPixels * 1.0);
...
void RegisterImages_MIBSpline(ImageType::Pointer & fixed_image, ImageType::Pointer & moving_image, TransformTypeBSpline::Pointer & out_transform) {
	...
	unsigned int numberOfGridNodesInOneDimension = 13;
